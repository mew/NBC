...
/**
 * ScrollComponent with minor tweaks.
 * @see club.sk1er.elementa.components.ScrollComponent
 */
class ChatScrollerComponent @JvmOverloads constructor(
        emptyString: String = "",
        private val scrollOpposite: Boolean = false,
        private val innerPadding: Float = 0f,
        private val scrollIconColor: Color = Color.WHITE,
        private val allowPhysicalScrolling: Boolean = true
) : UIContainer() {
    val actualHolder = UIContainer().constrain {
    ...
    override fun draw() {
        if (needsUpdate) {
            ...
            actualHolder.animate {
                ...
                val actualHeight = abs(calculateActualHeight())
                offset = if (offset < 0) 0f else if (offset > actualHeight) actualHeight else offset
                setYAnimation(Animations.IN_SIN, 0.2f, offset.pixels())
            }
    ...
    fun filterChildren(filter: (component: UIComponent) -> Boolean) {
        ...
        CopyOnWriteArrayList(allChildren.filter(filter).ifEmpty { listOf(emptyText) }).forEach { actualHolder.addChild(it) }
        actualHolder.children.forEach { it.parent = actualHolder }

        needsUpdate = true
    }

    fun triggerScroll(delta: Int): Unit = onScroll(delta, false)
    ...
    private fun onScroll(delta: Int, physical: Boolean = true) {
        if (!physical || allowPhysicalScrolling) {
            offset += (delta * 15)
    ...
    fun setChildren(components: List<UIComponent>) = apply {
        ...
        CopyOnWriteArrayList(components.ifEmpty { listOf(emptyText) }).forEach { actualHolder.addChild(it) }
        actualHolder.children.forEach { it.parent = actualHolder }
