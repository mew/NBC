...
/**
 * Elementa's UITextInput is kinda buggy and doesn't have everything I need.
 * @see club.sk1er.elementa.components.UITextInput
 */
open class TextInputComponent @JvmOverloads constructor(
        private val parentWidth: PixelConstraint,
        private val placeholder: String = "",
        var wrapped: Boolean = true,
        var shadow: Boolean = true,
        private val restrictions: Restrictions = Restrictions.NONE
) : UIComponent() {

    private val placeholderWidth = UniversalGraphicsHandler.getStringWidth(placeholder).toFloat()
    private val internalMouseClickAction = { _: Float, _: Float, _: Int ->
        if (active) {
            // TODO move cursor
        }
    }
    ...
    //var maxWidth: WidthConstraint = width//UniversalGraphicsHandler.getStringWidth(placeholder).pixels()
    //var minWidth: WidthConstraint = width//UniversalGraphicsHandler.getStringWidth(placeholder).pixels()
        ...
        onKeyType { typedChar, keyCode ->
            ...
            if (keyCode == 1) {
                ...
                when (restrictions) {
                    Restrictions.COLOUR -> {
                        when {
                            text != "" && text.toInt() >= 255 -> return@onKeyType
                            chars.contains(typedChar) -> {
                                if (text.toCharArray().size < 3) cursorLocation++
                                if ((text + typedChar).toInt() > 255) text = "255" else text += typedChar
                            }
                        }
                    }
                    Restrictions.FLOAT -> {
                        if (chars.contains(typedChar)) {
                            text += typedChar
                            cursorLocation++
                        } else if (typedChar == '.') {
                            if (!text.contains('.')) {
    ...
    private fun alignCursor(displayText: String = ""): List<String> {
        ...
        setWidth(width.pixels().minMax(parentWidth, parentWidth))
